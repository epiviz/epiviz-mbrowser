...
describe('mbrowser test', async () => {
    ...
    it('measurement according to project/collection/chart', async () => {
        ...
        const measuremnt_data = await page.$$eval('shadowDom/#measurement > epiviz-measurement-browser|#cardElem@#cardContainer > paper-card', (measurements) => {
            ...
            const chart_type = document.querySelector("#measurement > epiviz-measurement-browser").shadowRoot.querySelector("#modal > div.header > div:nth-child(3) > paper-dropdown").shadowRoot.querySelector("#dropdownMenu").shadowRoot.querySelector("#menuButton > div > paper-input").shadowRoot.querySelector("#input-3 > input[type=text]").value;
            return {
